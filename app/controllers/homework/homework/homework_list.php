<?php
	...
	$cond = "owner = '".$myUser['username']."'";

	$page_cnt = DB::selectCount("select count(*) from user_homework where ".$cond.";");
	...
	<tbody>
		<?php
			if($page_cnt<=0){
				...
				$sql = mysql_query("select * from user_homework where ".$cond." order by homework_id desc;");
				while($info = mysql_fetch_array($sql)){
					...
					$sqlb = mysql_query("select * from homework_index where id = ".$info['homework_id']." ;");
					$infob = mysql_fetch_array($sqlb);
		?>

		<tr>
			<td><?php echo $cnt; ?></td>
			<td><?php echo $infob['title']; ?></td>
			<td><a href="javascript:homeworkresult(<?php echo $infob['id']; ?>)">同学完成情况</a> <a href="javascript:homeworkdetail(<?php echo $infob['id']; ?>)">查看</a></td>
		</tr>
