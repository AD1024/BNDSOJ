<?php
	...
	$page_cnt = DB::selectCount("select count(*) from user_group where ".$cond.";");
...
<table class="table table-hover table-striped table-text-center">
	...
	<tbody>
		...
		<tr>
			...
			<td><?php echo DB::selectCount("select count(*) from user_group_map where group_id = '".$info['id']."';");; ?></td>
			<td><a href="javascript:managegroup(<?php echo $info['id']; ?>)">管理用户组</a> <a href="javascript:deletegroup(<?php echo $info['id'].', \''.$info['group_name'].'\''; ?>)">删除</a></td>
		</tr>
